...
MIGRATIONS=[ ("ida_kernwin.Choose2","ida_kernwin.Choose"),
...
def log(param, line):
    print("[*] {} {}".format(param, line))


def match_pattern(param, line, verbose):
    real_pattern = param
    if verbose: log(param, line)
    if "." in param:
        real_pattern = param.split(".")[-1]
    if "(" in real_pattern:
        real_pattern = real_pattern.split("(")[0]

    return re.search("\W{}\W".format(real_pattern),line)

def migrate(input, verbose):
    with open(input,"r") as input_file:
        lines = input_file.readlines()
        line_no = 1
        for line in lines:
            for pattern in MIGRATIONS:
                if match_pattern(pattern[0], line, verbose):
                    print("%s:%d: %s => %s" % (input, line_no, pattern[0], pattern[1]))
            line_no += 1
...
if __name__ == '__main__':
    ...
    migrate(args.input,args.verbose)
